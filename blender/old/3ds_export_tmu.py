...
# coding: utf-8
""" 
Name: '3ds for Trackmania .'
Blender: 248
Group: 'Export'
Tooltip: 'Export to .3ds file for use in Trackmania United.'
"""

__author__ = ["Glauco Bacchi", "Campbell Barton", "Bob Holcomb", "Richard Lärkäng", "Damien McGinnes", "Mark Stijnman"]
__url__ = ("blenderartists.org", "www.blender.org", "www.gametutorials.com", "lib3ds.sourceforge.net/")
__version__ = "Undecided"
__bpydoc__ = """\

3ds Exporter for Trackmania.

This script exports a 3ds file primarily for use in the game Trackmania United.

My modifications for Trackmania use have left the code that deals with parented objects as I found it. I think it best to avoid their use. 
Empties do not work correctly either.

This script is based on the KEX version (0.98a) of the 3ds exporter which in turn was based on version 0.90a available with blender.
...
def make_mesh_chunk(mesh, materialDict, ob, name_to_id, name_to_scale, name_to_pos, name_to_rot): 
	...
	if (ob.parent == None) or (ob.parent.name not in name_to_id):
		...
		#zzz this was originally matrix_pos = (-name_to_pos[ob.name][0],-name_to_pos[ob.name][1],-name_to_pos[ob.name][2]) 
		...
		#zzz this code has been left as found, Glauco Bacchi
...
def make_kf_obj_node(obj, name_to_id, name_to_scale, name_to_pos, name_to_rot):
	...
	if (parent == None) or (parent.name not in name_to_id):
		...
		#this was originally as follows, Glauco Bacchi		pivot_pos = (name_to_pos[name][0],name_to_pos[name][1],name_to_pos[name][2])
	...
	# 4KEX: The mesh has already been positioned around the object centre and scaled appropriately zzz
	if (parent == None) or (parent.name not in name_to_id):

		obj_size = (1.0,1.0,1.0) #zzz this was as I found it and works for TMU but means we do not apply any scaling at all, Glauco Bacchi
